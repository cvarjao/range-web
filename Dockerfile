...
RUN echo "Installing nodejs" &&  mkdir -p /opt/node && cd /opt/node && curl -L "${NODE_URI}" | tar -xvz

RUN echo "Installing caddy" && mkdir -p /opt/caddy && cd /opt/caddy && curl -L "${CADDY_URI}" | tar -xvz
